...
KEY RULES:
- SALE qty_change is NEGATIVE → use ABS(qty_change) or -SUM(qty_change) for units sold
- RETURN qty_change is POSITIVE → use SUM(qty_change) directly, NEVER use ABS() or negative filter for returns
- RECEIPT qty_change is POSITIVE → stock received into location
- TRANSFER_OUT qty_change is NEGATIVE; TRANSFER_IN qty_change is POSITIVE
- NEVER filter qty_change < 0 for RETURN — it is always positive
...
- TIMEZONE: ALL date grouping MUST use IST: DATE(im.moved_at AT TIME ZONE 'Asia/Kolkata') AS sale_date
- LIMIT rules: GROUP BY colour/size/sku/location breakdown → LIMIT 200; top-N queries → LIMIT N; single aggregate → no LIMIT; user says "all" → no LIMIT
- COCO=company stores, FOFO=franchise stores
- Dead stock = qty_180_plus; Low stock = qty_available < safety_stock

COLOR MATCHING:
- Exact color (black,white,navy) → s.color_name='BLACK' (uppercase exact)
...
- "how was sales / how did X perform / sales overview / sales analysis" → ALWAYS return a multi-row breakdown by colour OR by day (DATE IST), NEVER a single aggregate. Include: units_sold, revenue (units*mrp), and a grouping column.
- "by colour / colour-wise / color breakdown / colour split" → GROUP BY s.color_name (NOT by date). This includes shades of black: BLACK, JET BLACK, CHARCOAL BLACK, CARBON BLACK etc. — use s.color_name ILIKE '%BLACK%' for all black shades.
- "by day / daily / day-wise / date-wise" → GROUP BY DATE(im.moved_at AT TIME ZONE 'Asia/Kolkata')
- "total / count / how many" (single metric, no "by" clause) → single aggregate is fine
...
→ RETURN is POSITIVE qty_change. Never use ABS() or qty_change<0 for returns.
...
→ {"sql":"SELECT DATE(im.moved_at AT TIME ZONE 'Asia/Kolkata') AS sale_date, COALESCE(SUM(ABS(im.qty_change)),0) AS units_sold, COALESCE(SUM(ABS(im.qty_change)*s.mrp),0) AS revenue, COUNT(DISTINCT im.location_id) AS stores_active FROM inventory_movements im JOIN skus s ON s.id=im.sku_id JOIN locations l ON l.id=im.location_id WHERE im.movement_type='SALE' AND l.is_active=true AND s.is_active=true AND im.moved_at>='2025-03-08'::date AND im.moved_at<'2025-03-15'::date GROUP BY DATE(im.moved_at AT TIME ZONE 'Asia/Kolkata') ORDER BY sale_date","explanation":"Daily sales breakdown for first week of Holi 2025 (Mar 8-14)"}

Q:"black units sold by colour from 1 may 2025 to 5 may 2025"
→ User wants colour-wise breakdown of black shades. GROUP BY s.color_name, not by date.
→ {"sql":"SELECT s.color_name, COALESCE(SUM(ABS(im.qty_change)),0) AS units_sold, COALESCE(SUM(ABS(im.qty_change)*s.mrp),0) AS revenue FROM inventory_movements im JOIN skus s ON s.id=im.sku_id JOIN locations l ON l.id=im.location_id WHERE im.movement_type='SALE' AND s.color_name ILIKE '%BLACK%' AND l.is_active=true AND s.is_active=true AND im.moved_at>='2025-05-01'::date AND im.moved_at<'2025-05-06'::date GROUP BY s.color_name ORDER BY units_sold DESC","explanation":"Black colour shades sold from May 1-5 2025, grouped by colour variant"}
    ...
    if (queryResult.rows.length === 1) {
      try {
        ...
        const { result: bdResult } = await generateWithFallback(
          `Question: "${question}"
Total result: ${JSON.stringify(aggregate)}
Generate a SQL breakdown of the SAME filters grouped by l.name AS store_name, l.city, l.type.
Select: l.name AS store_name, l.city, l.type, and ${metricCol === 'units_returned' ? 'COALESCE(SUM(im.qty_change),0) AS units_returned' : metricCol === 'units_sold' ? 'COALESCE(SUM(ABS(im.qty_change)),0) AS units_sold, COALESCE(SUM(ABS(im.qty_change)*s.mrp),0) AS revenue' : `COALESCE(SUM(ABS(im.qty_change)),0) AS units`}.
Same WHERE clause, same date range, same SKU filters. ORDER BY the metric DESC LIMIT 10.
Return JSON: {"sql":"..."}`,
          buildSchemaContext(),
